...
/**
 * Controller for Customer Deposit Operations
 */
@Controller
...
public class DepositController {
	...
	/**
	 * Handle deposit request (POST only)
	 * GET request handled by CustomerViewController
	 */
	@PostMapping
	public String handleDeposit(@RequestParam("amount") String amountStr,
	                            HttpSession session,
	                            Model model) {
		...
		try {
			...
			// Process deposit (validates amount, customer status, etc.)
			Transaction transaction = customerService.depositMoney(customer.getId(), amount);
			...
			return "customer/deposit-success";
