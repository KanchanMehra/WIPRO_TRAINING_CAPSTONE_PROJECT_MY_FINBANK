...
<head>
	...
	<title>MyFin Bank - Chat Support</title>
...
<nav class="navbar navbar-expand-lg navbar-custom">
	<div class="container-fluid">
		...
		<div class="d-flex align-items-center">
			<span class="navbar-text me-3">${customer.firstName} ${customer.lastName}</span>
			<a href="${pageContext.request.contextPath}/customer/dashboard" class="btn btn-outline-light btn-sm">Back to Dashboard</a>
		</div>
	</div>
</nav>
...
<script>
	...
	function sendJoinMessage() {
		if (stompClient && connected) {
			const joinMessage = {
				...
				roomId: roomId,
				message: customerName + ' joined the chat',
				messageType: 'JOIN'
			};
	...
	function showMessage(message) {
		...
		if (message.messageType === 'JOIN' || message.messageType === 'LEAVE' || message.messageType === 'SYSTEM') {
			...
			const isOwn = String(message.senderId) === String(customerId) && message.senderType === 'CUSTOMER';
	...
	function loadChatHistory() {
		fetch('${pageContext.request.contextPath}/api/chat/history/' + roomId)
	...
	window.addEventListener('beforeunload', function() {
		if (stompClient && connected) {
			const leaveMessage = {
				...
				senderName: customerName,
				roomId: roomId,
				message: customerName + ' left the chat',
				messageType: 'LEAVE'
			};
